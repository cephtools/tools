#!/bin/bash
# bluestore_min_alloc_size is declared 'uint' while its _hdd/_ssd siblings
...
mkosd $WORK/mas_hdd 2G --bluestore-min-alloc-size-hdd=64K && echo mkfs ok
echo "== mkfs with bluestore_min_alloc_size=64K"
mkosd $WORK/mas_generic 2G --bluestore-min-alloc-size=64K && echo mkfs ok
grep -m2 -i "power of 2\|min_alloc_size" $WORK/mas_generic/mkfs.log
