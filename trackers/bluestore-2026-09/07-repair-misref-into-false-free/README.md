...
| Real-world | Real trigger path is `ceph-bluestore-tool repair` on a store that already has both errors; that corruption can only be created with BlueStore test hooks, so it is demonstrated at the ObjectStore level |
