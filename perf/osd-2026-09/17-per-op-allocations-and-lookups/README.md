...
| Risk | low to medium |
| Status | code analysis (verified in the source); the data digest (F5) was measured: no measurable effect |
...
| F5 | `osd_skip_data_digest` defaults to false, so every whole-object write and read computes crc32c over the data on top of BlueStore's own checksum | `common/options` default `false` | config: `osd_skip_data_digest = true` — measured: OSD CPU per op −1.6% to +3.6%, within noise (`results/2026-09-25-ab4.txt`; the option was passed to the OSDs, its effect was not checked separately) |
